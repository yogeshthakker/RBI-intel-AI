...
def extract_batch(
    ...
    # Recovery mode: retry only clauses that are still pending AND do not
    # already have a successful/skip result in the JSONL cache. This leaves
    # previously kept/skipped clauses untouched and resubmits prior failures.
    if retry_failed:
        cached_ids: set[str] = set()
        for doc_id in doc_ids:
            cached_ids.update(_already_cached(doc_id))
        before = len(rows)
        rows = [r for r in rows if r['id'] not in cached_ids]
        if not quiet:
            print(f'[batch] retry-failed: {len(rows)}/{before} clauses need retry', file=sys.stderr)

    if not rows:
        note = 'nothing failed to retry' if retry_failed else 'nothing pending — pass --force to redo'
        return {'clauses': 0, 'kept': 0, 'skipped': 0, 'failed': 0, 'note': note}
